...
describe("DealCard", () => {
  ...
  it("shows a red circular next-action indicator when the activity is overdue", () => {
    ...
    const ind = screen.getByRole("img", { name: "activity overdue" });
  ...
  it("shows a GREEN circular indicator when an activity is due today", () => {
    ...
    const ind = screen.getByRole("img", { name: "activity due today" });
  ...
  it("shows a YELLOW warning indicator when there is no next activity (schedule-one nudge, not gray)", () => {
    ...
    const ind = screen.getByRole("img", { name: "no activity planned" });
    expect(ind.className).toContain("rounded-full");
  ...
  it("labels the next-action indicator 'activity overdue' when overdue (a11y non-color cue)", () => {
    render(
      <DealCard
        card={{
          ...baseCard,
    ...
    // State conveyed via aria-label, not color alone.
    expect(screen.getByRole("img", { name: "activity overdue" })).not.toBeNull();
  });
});
