// The per-operator SQL branches for the leads filter compiler, split out of leadFilter.ts to keep
// that module at the allow-list and compile shape. Every value here is a bound parameter and every
// operator string is a constant; see leadFilter.ts for the full invariant list.
import { type SQL, sql } from "drizzle-orm";
import { AppError, ERROR_IDS } from "@/constants/errorIds";
import type { LeadFilterOp } from "./leadFilter";

// neq is IS DISTINCT FROM, not <>: `NULL <> 5` is NULL, which would drop every lead with no value
// from an "is not" filter.
const OP_RAW = {
  eq: "=",
...
export function requireValue(
  ...
  if (value === undefined) {
    throw new AppError(ERROR_IDS.LEAD_FILTER_INVALID, "Missing value for a leads filter op", {
      op,
    });
  }
  return value;
...
function requireScalar(op: LeadFilterOp, value: string | number | string[]): string | number {
  if (Array.isArray(value)) {
    throw new AppError(ERROR_IDS.LEAD_FILTER_INVALID, "List value on a scalar leads field", { op });
  }
  return value;
}
...
export function requireLabelValue(
  ...
  if (Array.isArray(value) && value.length === 0) {
    throw new AppError(ERROR_IDS.LEAD_FILTER_INVALID, "Empty list on a labels condition", { op });
  }
